...
class LoadData(object):
    ...
    def construct_data(self, loss_type):
        X_, Y_, Y_for_logloss, R_= self.read_data(self.trainfile)
        if loss_type == 'log_loss':
            Train_data = self.construct_dataset(X_, Y_for_logloss, R_)
        else:
            Train_data = self.construct_dataset(X_, Y_, R_)
        #print("Number of samples in Train:" , len(Y_))

        X_, Y_ , Y_for_logloss,R_= self.read_data(self.validationfile)
        if loss_type == 'log_loss':
            Validation_data = self.construct_dataset(X_, Y_for_logloss, R_)
        else:
            Validation_data = self.construct_dataset(X_, Y_, R_)
        #print("Number of samples in Validation:", len(Y_))

        X_, Y_ , Y_for_logloss,R_ = self.read_data(self.testfile)
        if loss_type == 'log_loss':
            Test_data = self.construct_dataset(X_, Y_for_logloss, R_)
        else:
            Test_data = self.construct_dataset(X_, Y_, R_)
    ...
    def read_data(self, file):
        ...
        while line:
            ...
            R_.append(1.0*float(items[-1]))
            X_.append([self.features[item] for item in items[1:-1]])
            line = f.readline()
        f.close()
        return X_, Y_, Y_for_logloss, R_

    def construct_dataset(self, X_, Y_, R_):
        Data_Dic = {}
        X_lens = [ len(line) for line in X_]
        indexs = np.argsort(X_lens)
        Data_Dic['Y'] = [ Y_[i] for i in indexs]
        Data_Dic['X'] = [ X_[i] for i in indexs]
        Data_Dic['R'] = [ R_[i] for i in indexs]
        return Data_Dic
