...
def parse_args():
    ...
    parser.add_argument('--dataset', nargs='?', default='web',
                        help='Choose a dataset.')
    parser.add_argument('--valid_dimen', type=int, default=15,
    ...
    parser.add_argument('--keep', nargs='?', default='[0.7,0.6]',
...
class AFM(BaseEstimator, TransformerMixin):
    def __init__(self, features_M, pretrain_flag, save_file, attention, hidden_factor, valid_dimension, activation_function, num_variable,
        ...
        self.train_aucscore, self.valid_aucscore, self.test_aucscore = [], [], []
    ...
    def _init_graph(self):
        ...
        with self.graph.as_default():  # , tf.device('/cpu:0'):
            ...
            self.train_rulescore = tf.placeholder(tf.float32, shape=[None, 1],name="train_rulescore_afm")  # None * features_M
            ...
            print(self.nonzero_embeddings)
            element_wise_product_list = []#这里是元素之间的积！！！！！！！！！！！！
            count = 0
            for i in range(0, self.valid_dimension):
                for j in range(i+1, self.valid_dimension):
                    element_wise_product_list.append(tf.multiply(self.nonzero_embeddings[:,i,:], self.nonzero_embeddings[:,j,:]))#如果i和j是category和category则插入权重！！
                    count += 1
            ...
            self.interactions = tf.reduce_sum(self.element_wise_product, 2, name="interactions")#交叉特征
            # _________ MLP Layer / attention part _____________
            num_interactions = self.valid_dimension*(self.valid_dimension-1)/2
            if self.attention:
                #self.train_labels = tf.reshape(self.train_labels, shape=[1, -1, 1])
                #self.train_rulescore=tf.reshape(self.train_rulescore,shape=[-1,-1,1])
                #self.train_features = tf.reshape(self.train_features, shape=[1, -1, 1])
                #self.train_labels = tf.reshape(self.train_labels, shape=[-1, -1, 1])
                self.attention_mul = tf.reshape(tf.matmul(tf.reshape(self.element_wise_product, shape=[-1, self.hidden_factor[1]]), \
                    self.weights['attention_W']), shape=[-1, int(num_interactions), int(self.hidden_factor[0])])
                ...
                self.train_rulescore = tf.nn.softmax(self.train_rulescore)
                self.attention_out = tf.nn.dropout(self.attention_out, self.dropout_keep[0]) # dropout
                print(self.attention_out)
                rule_of_a_r=[]
                rule_of_a_r.append(tf.nn.softmax(tf.add(self.train_rulescore*100,self.attention_out[:,:,0])))
                self.attention_out=tf.stack(rule_of_a_r)
                self.attention_out=tf.reshape(self.attention_out,shape=[-1,105,1])
                #self.attention_out = tf.nn.dropout(self.attention_out, self.dropout_keep[0]) # dropout
                #self.ruleaf = tf.transpose(self.ruleaf, perm=[1, 2, 0],name="ruleaf")
                #print(self.ruleaf)
                print(self.attention_out)
                # with tf.Session() as sess:
                #     print(sess.run(self.train_labels))
            ...
            #print(self.sess.run(self.attention_out))
            self.sess.run(init)
    ...
    def _initialize_weights(self):#权重，将权重存到图里面
        all_weights = dict()
        ...
        if self.pretrain_flag > 0 or self.micro_level_analysis:
            ...
            #all_weights['feature_rule'] = tf.Variable(ru, dtype=tf.float32, name='feature_rule',trainable=trainable)
            ...
            all_weights['feature_embeddings'] = tf.Variable(tf.random_normal([self.features_M, self.hidden_factor[1]], 0.0, 0.01),name='feature_embeddings', trainable=trainable)  # features_M * K
            #all_weights['feature_rule'] = tf.Variable(tf.random_normal([self.features_M, self.hidden_factor[1]], 0.0, 0.01),name='feature_rule', trainable=trainable)  # features_M * K
    ...
    def partial_fit(self, data):  # fit a batch
        feed_dict = {self.train_features: data['X'],self.train_rulescore: data['R'], self.train_labels: data['Y'], self.dropout_keep: self.keep, self.train_phase: True}
    ...
    def train(self, Train_data, Validation_data, Test_data):  # fit a dataset
        ...
        for epoch in range(self.epoch):
            t1 = time()
            self.shuffle_in_unison_scary(Train_data['X'], Train_data['Y'], Train_data['R'])
            ...
            self.train_aucscore.append(train_auc)
            self.valid_aucscore.append(valid_auc)
            if self.verbose > 0 and epoch%self.verbose == 0:
                print("Epoch %d [%.1f s]\ttrain_rmse=%.4f, validation_rmse=%.4f,train_auc=%.4f, validation_auc=%.4f  [%.1f s]"
                      %(epoch+1, pertime, train_result, valid_result,train_auc, valid_auc, sum_time))
    ...
    def evaluate(self, data):  # evaluate the results for an input set
        ...
        while len(batch_xs['X']) > 0:
            ...
            feed_dict = {self.train_features: batch_xs['X'], self.train_rulescore: [[r] for r in batch_xs['R']], self.train_labels: [[y] for y in batch_xs['Y']], self.dropout_keep: list(1.0 for i in range(len(self.keep))), self.train_phase: False}
        ...
        df = pd.DataFrame(predictions_bounded)
        # df.to_csv("outReAFM.csv", index=False)
        # df = pd.DataFrame(a_out)
        df.to_csv("AoutReAFM.csv", index=False)
        RMSE = math.sqrt(mean_squared_error(y_true, predictions_bounded))
        roc_score = roc_auc_score(y_true, predictions_bounded)
        #logloss=log_loss(y_true, predictions_bounded)
        #res = precision_recall_fscore_k(y_true, predictions_bounded, k=1, digs=2)
        return RMSE, roc_score
