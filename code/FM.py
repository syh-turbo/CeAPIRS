...
class FM(BaseEstimator, TransformerMixin):
    ...
    def _init_graph(self):
        ...
        with self.graph.as_default():  # , tf.device('/cpu:0'):
            ...
            self.train_rulescore = tf.placeholder(tf.float32, shape=[None, 1],name="train_rulescore_fm")  # None * features_M
            ...
            if self.micro_level_analysis:
                ...
                print(self.Feature_bias)
                self.Feature_bias= tf.reshape(self.Feature_bias, shape=[-1, 15, 1])
                rule_of_a_r = []
                rule_of_a_r.append(tf.nn.softmax(tf.add(self.train_rulescore, self.Feature_bias[:,:,0])))
                self.Feature_bias = tf.stack(rule_of_a_r)
                print(self.Feature_bias)
                #self.Feature_bias = tf.reduce_sum(tf.nn.embedding_lookup(self.weights['feature_bias'], self.train_features) , 1)  # None * 1
                #self.Feature_bias =(tf.add(self.train_rulescore, self.Feature_bias))
                Bias = self.weights['bias'] * tf.ones_like(self.train_labels)  # None * 1
                self.Feature_bias = tf.reduce_sum(self.Feature_bias, 2) # None * 1
                self.Feature_bias = tf.reduce_sum(self.Feature_bias, 0)  # None * 1
                self.Feature_bias = tf.reshape(self.Feature_bias, shape=[-1,1])
                print(self.Feature_bias)
    ...
    def evaluate(self, data):  # evaluate the results for an input set
        ...
        feed_dict = {self.train_features: data['X'],self.train_rulescore: [[r] for r in data['R']], self.train_labels: [[y] for y in data['Y']], self.dropout_keep: 1.0, self.train_phase: False}
        predictions = self.sess.run((self.out), feed_dict=feed_dict)
        y_pred = np.reshape(predictions, (num_example,))
        df = pd.DataFrame(y_pred)
        df.to_csv("outReFM.txt", index=False)
...
def evaluate(args):
    ...
    train_rulescore = pretrain_graph.get_tensor_by_name('train_rulescore_fm:0')
    ...
    # start evaluation
    num_example = len(data['Y'])
    feed_dict = {train_features: data['X'],train_rulescore: [[r] for r in data['R']], train_labels: [[y] for y in data['Y']], dropout_keep: 1.0, train_phase: False}
